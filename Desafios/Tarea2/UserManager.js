...
class UserManager {
    ...
    createUser = async (user) =>{
        if(!user) return {status: "error", message:"User Missing"}
        if(!user.first_name || !user.user_name || !user.mail) return {status: "error", error: "missing field"}
        try{
            if(fs.existsSync(pathToUsers)){
                ...
                return {status: "success", message:"First user created"}
            }
        }catch(err){
            return {status: "error", message: "va igual de bien que el país"}
        }
    }
    ...
    findAllUsers = async () =>{
        if(fs.existsSync(pathToUsers)){
            let data = await fs.promises.readFile(pathToUsers, "utf-8", null, 2);
            let users = JSON.parse(data);
            return {status: "success", usersFound: users}
        }
    ...
    findById = async (id) =>{
        ...
        if(fs.existsSync(pathToUsers)){
            ...
            if (userFound) return { status:"success", message: userFound }
